...
class GigaGAN(nn.Module):
    ...
    def train_discriminator_step(
        ...
        if self.max_grad_norm is not None:
            self.accelerator.clip_grad_norm_(self.D.parameters(), self.max_grad_norm)

        self.D_opt.step()

        if self.need_vision_aided_discriminator:
            if self.max_grad_norm is not None:
                self.accelerator.clip_grad_norm_(self.VD.parameters(), self.max_grad_norm)
            self.VD_opt.step()
    ...
    def train_generator_step(
        ...
        if self.max_grad_norm is not None:
            self.accelerator.clip_grad_norm_(self.G.parameters(), self.max_grad_norm)
    ...
    def forward(
        ...
        *,
        ...
        # 仅主进程写进度条，避免多卡各打一条；重定向到 train.log 时仍会输出（可能一行步进一条）
        for _ in tqdm(range(steps), initial=self.steps.item(), disable=not self.is_main):
            steps = self.steps.item()
            ...
            # 注意：不再使用 is_first_step 触发存盘，避免多卡时首步存盘耗时过长；首段密集存盘仍由 early_save_* 控制
            if self.is_main and (divisible_by(steps, self.save_and_sample_every) or (steps <= self.early_save_thres_steps and divisible_by(steps, self.early_save_and_sample_every))):
                self.save_sample(batch_size, dl_iter)

            # 仅主进程跑 save_sample 会耗时；其他 rank 必须在此等待，否则步进与 DDP 不同步导致死锁
            self.accelerator.wait_for_everyone()
